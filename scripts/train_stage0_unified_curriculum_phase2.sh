#!/bin/bash
# Stage 0 Unified Curriculum — Phase 2 (600 steps, cmd switch enabled, low switch prob).
#
# Usage:
#   bash scripts/train_stage0_unified_curriculum_phase2.sh [MAX_ITERS] [CHECKPOINT]
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  Stage0 Unified Curriculum — Phase 2                     ║"
echo "╠══════════════════════════════════════════════════════════╣"
echo "║  开始时间:    $TIMESTAMP"
echo "║  最大迭代:    $MAX_ITERS"
echo "║  task:        HumanoidAMPUnifiedCurriculum_phase2"
...
$PYTHON train_curriculum_unified.py \
  task=HumanoidAMPUnifiedCurriculum_phase2 \
...
if [ $EXIT_CODE -eq 0 ]; then
  echo "✅ Phase 2 完成. Checkpoints: $(pwd)/runs/"
else
  echo "❌ Phase 2 退出码: $EXIT_CODE"
fi
exit $EXIT_CODE
