...
EXTRA_BUNDLE="${2:-${ROOT}/runtime/hora_cuda_missing_cu126.tar.zst}"
...
for required in "${BUNDLE}" "${EXTRA_BUNDLE}"; do
  if [[ ! -f "${required}" ]]; then
    echo "missing offline HORA environment bundle: ${required}" >&2
    exit 1
  fi
done
if [[ -e "${TARGET}" ]]; then
...
for archive in "${BUNDLE}" "${EXTRA_BUNDLE}"; do
"${REPO_ROOT}/.venv/bin/python" - "${archive}" "${TEMPORARY_TAR}" <<'PY'
