...
valid = valid and manifest.get("schema_version") == 3
...
if [[ "${DATA_READY}" == false ]]; then
  ...
  echo "qualified v3 tracker rollouts already exist; replay validation will run again"
fi
