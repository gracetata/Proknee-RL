...
# HumanoidAMPUnified curriculum: discrete velocity grid, walk/run demo split by speed threshold.
...
class HumanoidAMPUnifiedCurriculum(HumanoidAMPUnified):
    """Unified AMP with 0.1 m/s velocity grid, AMP demos from walk vs run by velocityMotionThreshold."""

    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        env = cfg["env"]
        self._velocity_grid_step = float(env.get("velocityGridStep", 0.1))
        self._velocity_max = float(env.get("velocityMax", 2.5))
        self._velocity_motion_threshold = float(env.get("velocityMotionThreshold", 1.5))
    ...
    def fetch_amp_obs_demo(self, num_samples):
        ...
        motion_ids = np.where(
            v_vals <= self._velocity_motion_threshold, self._walk_motion_index, self._run_motion_index
        )
